...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: rawReports }, { data: results }] = await Promise.all([
    supabase.from("profiles").select("first_name").eq("id", user.id).maybeSingle(),
    supabase.from("lab_reports").select("id,sample_date,lab_name,status,created_at,extraction_version").order("sample_date", { ascending: false }),
    supabase.from("lab_results").select("id,value_numeric,unit_raw,unit_canonical,flag,created_at,biomarker_catalog(slug,display_name,category),lab_reports(sample_date,lab_name,created_at,extraction_version)"),
  ]);
