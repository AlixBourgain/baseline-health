...
import { dedupeReports } from "@/lib/health/result-selection";

export default async function BloodTestsPage() {
  await requireUser();
  const supabase = await createClient();
  const { data: rawReports } = await supabase.from("lab_reports").select("id,sample_date,lab_name,original_filename,status,created_at,extraction_version").order("sample_date", { ascending: false });
