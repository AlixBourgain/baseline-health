...
export default async function BiomarkerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: rawResults } = await supabase.from("lab_results").select("id,value_numeric,unit_raw,unit_canonical,reference_low,reference_high,flag,lab_reports(sample_date,lab_name)").eq("biomarker_id", biomarker.id);
  const results = (rawResults ?? []).slice().sort((a: any, b: any) => new Date(a.lab_reports.sample_date).getTime() - new Date(b.lab_reports.sample_date).getTime());
  ...
  return <div className="mx-auto max-w-5xl"><p className="text-sm text-neutral-500">{biomarker.category}</p><div className="mt-1 flex flex-wrap items-end justify-between gap-4"><h1 className="text-4xl font-semibold tracking-tight">{biomarker.display_name}</h1>{latest ? <p className="text-3xl font-semibold">{latest.value_numeric} <span className="text-sm font-normal text-neutral-500">{latest.unit_canonical || latest.unit_raw}</span></p> : null}</div>
    <Card className="mt-8 p-6"><h2 className="text-sm font-medium">Évolution dans le temps</h2><div className="mt-5"><TrendChart data={chartData} unit={biomarker.canonical_unit}/></div></Card>
